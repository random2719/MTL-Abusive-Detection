...
#sentiment_map = {0: 'neither', 1: 'offensive', -1: 'hatespeech'}
sentiment_map = {0: 'neither', 1: 'offensive'}
...
def main():
    ...
    dev_file = os.path.join(args.data_path, 'dev.raw')

    # raw -> txt
    train_sentences = read_file(train_file)
    test_sentences = read_file(test_file)
    dev_sentences = read_file(dev_file)
    ...
    dev_sentences_with_dep = get_dependencies(os.path.join(args.data_path, 'dev.txt'), predictor)
    ...
    syntaxInfo2json(dev_sentences, dev_sentences_with_dep, os.path.join(args.data_path, 'dev.txt'))
