...
def load_datasets_and_vocabs(args):
    train, dev,test = get_dataset(args.dataset_name)
    ...
    _, dev_all_unrolled, _, _ = get_rolled_and_unrolled_data(dev, args)
    ...
    logger.info('Dev set size: %s', len(dev_all_unrolled))
    ...
    dev_dataset = ASBA_Depparsed_Dataset(
        dev_all_unrolled, args, word_vocab, dep_tag_vocab, pos_tag_vocab)
    test_dataset = ASBA_Depparsed_Dataset(
        test_all_unrolled, args, word_vocab, dep_tag_vocab, pos_tag_vocab)

    return train_dataset,dev_dataset, test_dataset, word_vocab, dep_tag_vocab, pos_tag_vocab
...
def get_dataset(dataset_name):
    ...
    #
    hatespeech_train = 'data/hatespeech/train_biaffine.json'
    hatespeech_test = 'data/hatespeech/test_biaffine.json'
    hatespeech_dev = 'data/hatespeech/dev_biaffine.json'

    hatespeech_not_off_train = 'data/hatespeech_not_off/train_biaffine.json'
    hatespeech_not_off_test = 'data/hatespeech_not_off/test_biaffine.json'
    hatespeech_not_off_dev = 'data/hatespeech_not_off/dev_biaffine.json'

    hatespeech_off_hate_train = 'data/hatespeech_off_hate/train_biaffine.json'
    hatespeech_off_hate_test = 'data/hatespeech_off_hate/test_biaffine.json'
    hatespeech_off_hate_dev = 'data/hatespeech_off_hate/dev_biaffine.json'


    # ds_train = {'hatespeech': hatespeech_train}
    # ds_test = {'hatespeech': hatespeech_test}
    # ds_dev = {'hatespeech': hatespeech_dev}
    ...
    ds_train = {'rest': rest_train,
                'laptop': laptop_train, 'twitter': twitter_train,'hatespeech': hatespeech_train,'hatespeech_not_off':hatespeech_not_off_train,'hatespeech_off_hate':hatespeech_off_hate_train}
    ds_dev = {'rest': rest_test,
               'laptop': laptop_test, 'twitter': twitter_test, 'hatespeech': hatespeech_dev,'hatespeech_not_off':hatespeech_not_off_dev,'hatespeech_off_hate':hatespeech_off_hate_dev}
    ds_test = {'rest': rest_test,
               'laptop': laptop_test, 'twitter': twitter_test,'hatespeech': hatespeech_test,'hatespeech_not_off':hatespeech_not_off_test,'hatespeech_off_hate':hatespeech_off_hate_test}
    ...
    dev = list(read_sentence_depparsed(ds_dev[dataset_name]))
    #test = list(read_sentence_depparsed(ds_test[dataset_name]))
    
    data=train+dev
    random.shuffle(data)
    new_train=data[0:int(0.8*len(data))]
    new_dev=data[int(0.8*len(data)):]

    # data=train+dev+test
    # random.shuffle(data)
    # data_train=data[0:-2999]
    # new_test=data[-2999:]

    # new_train=data[0:int(0.8*len(new_train))]
    # new_dev=data[int(0.8*len(new_train)):]
    
    logger.info('# Read %s Train set: %d', dataset_name, len(train))
    logger.info("# Read %s Dev set: %d", dataset_name, len(dev))
    test = list(read_sentence_depparsed(ds_test[dataset_name]))
    logger.info("# Read %s Test set: %d", dataset_name, len(test))

    #dev = list(read_sentence_depparsed(ds_dev[dataset_name]))
    #logger.info("# Read %s Test set: %d", dataset_name, len(dev))
...
def get_rolled_and_unrolled_data(input_data, args):
    ...
    #sentiments_lookup = {'neither': 0, 'offensive': 1, 'hatespeech': 1}
    #node_sentiment_lookup={'neither': 0, 'offensive': 1}
    sentiments_lookup = {'neither': 0, 'offensive': 1}
    #sentiments_lookup = {'offensive': 0, 'hatespeech': 1}
        ...
        for i in range(len(e['aspect_sentiment'])):
            ...
            all_unrolled.append(
                {'sentence': e['tokens'], 'tags': e['tags'], 'pos_class': pos_class, 'aspect': aspect, 'sentiment': sentiment,
                 ...
                 'node_labels':e['node_labels'],'important_word':e['important_word']})
...
def load_and_cache_vocabs(data, args):
    ...
    '''
    pkls_path = os.path.join('data/output-hatespeech', 'pkls')
    ...
    if args.embedding_type == 'glove':
        cached_word_vocab_file = os.path.join(
            pkls_path, 'cached_{}_{}_word_vocab.pkl'.format('hatespeech', args.embedding_type))
        ...
        cached_word_vecs_file = os.path.join(pkls_path, 'cached_{}_{}_word_vecs.pkl'.format(
            'hatespeech', args.embedding_type))
    ...
    cached_dep_tag_vocab_file = os.path.join(
        pkls_path, 'cached_{}_dep_tag_vocab.pkl'.format('hatespeech'))
    ...
    cached_pos_tag_vocab_file = os.path.join(
        pkls_path, 'cached_{}_pos_tag_vocab.pkl'.format('hatespeech'))
...
def build_text_vocab(data, vocab_size=100000, min_freq=2):
    ...
    itos = ['[PAD]', '[UNK]']    #<pad>是词汇表index=0,<unk>是词汇表index1
...
class ASBA_Depparsed_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #     e['dep_rel_ids'], e['predicted_heads'], e['aspect_position'], e['dep_dir_ids'],e['node_labels'],e['important_word_ids']
        
        items = e['dep_tag_ids'], \
            e['pos_class'], e['text_len'], e['aspect_len'], e['sentiment'],\
            e['dep_rel_ids'], e['predicted_heads'], e['aspect_position'], e['dep_dir_ids'],e['node_labels']
    ...
    def convert_features_bert(self, i):
        ...
        if self.args.pure_bert:
            ...
            #self.data[i]['important_word_ids'] = input_aspect_ids
    ...
    def convert_features(self):
        ...
        for i in range(len(self.data)):
            ...
            #print('aspect=', self.data[i]['aspect'])
            #print('node_labels=',self.data[i]['node_labels'])
            if self.args.embedding_type == 'glove':
                ...
                self.data[i]['important_word_ids'] = [self.word_vocab['stoi'][w]
                                                for w in self.data[i]['important_word']]
            elif self.args.embedding_type == 'elmo':
...
def my_collate(batch):
    ...
    '''
    sentence_ids, aspect_ids, dep_tag_ids, pos_class, text_len, aspect_len, sentiment, dep_rel_ids, dep_heads, aspect_positions, dep_dir_ids,node_labels,important_word_ids = zip(
    ...
    #print('node_labels=',type(node_labels))

    # for item in node_labels:
    #     node_labels=torch.tensor(item.cpu().detach().numpy())

    #node_labels= torch.tensor([item.cpu().detach().numpy() for item in node_labels]).cuda() 

    #node_labels=torch.tensor(node_labels)

    # Pad sequences.
    node_labels = pad_sequence(
        node_labels, batch_first=True, padding_value=-1)
    #print('node_labels=',node_labels)
    sentence_ids = pad_sequence(
        sentence_ids, batch_first=True, padding_value=0)
    important_word_ids = pad_sequence(
        important_word_ids, batch_first=True, padding_value=0)
    ...
    node_labels=node_labels[sorted_idx]
    important_word_ids=important_word_ids[sorted_idx]

    return sentence_ids, aspect_ids, dep_tag_ids, pos_class, text_len, aspect_len, sentiment, dep_rel_ids, dep_heads, aspect_positions, dep_dir_ids,node_labels,important_word_ids
...
def my_collate_elmo(batch):
    ...
    important_word_ids=batch_to_ids(important_word_ids)
...
def my_collate_pure_bert(batch):
    ...
    input_cat_ids, segment_ids, dep_tag_ids, pos_class, text_len, aspect_len, sentiment, dep_rel_ids, dep_heads, aspect_positions, dep_dir_ids,node_labels,important_word_ids = zip(
        *batch)  # from Dataset.__getitem__()
    ...
    return input_cat_ids, segment_ids, dep_tag_ids, pos_class, text_len, aspect_len, sentiment, dep_rel_ids, dep_heads, aspect_positions, dep_dir_ids,node_labels,important_word_ids
...
def my_collate_bert(batch):
    ...
    Process bert feature
    '''
    input_ids, word_indexer, input_aspect_ids, aspect_indexer,input_cat_ids,segment_ids,  dep_tag_ids, pos_class, text_len, aspect_len, sentiment, dep_rel_ids, dep_heads, aspect_positions, dep_dir_ids,node_labels= zip(
        *batch)
    text_len = torch.tensor(text_len)
    aspect_len = torch.tensor(aspect_len)
    sentiment = torch.tensor(sentiment)
    ...
    #important_word_ids = pad_sequence(important_word_ids, batch_first=True, padding_value=0)
    ...
    #return input_ids, word_indexer, input_aspect_ids, aspect_indexer,input_cat_ids,segment_ids, dep_tag_ids, pos_class, text_len, aspect_len, sentiment, dep_rel_ids, dep_heads, aspect_positions, dep_dir_ids,node_labels,important_word_ids
    return input_ids, word_indexer, input_aspect_ids, aspect_indexer,input_cat_ids,segment_ids, dep_tag_ids, pos_class, text_len, aspect_len, sentiment, dep_rel_ids, dep_heads, aspect_positions, dep_dir_ids,node_labels
