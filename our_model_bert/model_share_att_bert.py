...
class Expert_node(nn.Module):
    ...
    def forward(self, input_ids, input_aspect_ids, word_indexer, aspect_indexer, input_cat_ids, segment_ids, pos_class,
        ...
        dep_out = [g(fmask, rel_adj, dep_feature).unsqueeze(1) for g in self.gat_dep]
        dep_out = torch.cat(dep_out, dim = 1) # (N, H, D)
        dep_out = dep_out.mean(dim = 1) # (N, D)
...
class Tower_node(nn.Module):
    ...
    def forward(self, node_feature_final):
        #节点分类
        #print('node_feature_shape=',node_feature_final.shape)
...
class GatingMechanism(nn.Module):
    def __init__(self, args, dep_tag_num, pos_tag_num):
        ...
        # gating 的参数
        self.gate_theta_1 = Parameter(torch.empty(1, 1168))
        self.gate_theta_2 = Parameter(torch.empty(1, 1168))
